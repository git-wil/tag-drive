...
export const tagsSlice = createSlice({
    ...
    reducers: {
        ...
        modifyTagMetadata: (state, action) => {
            ...
            Object.values(state.file_tags).forEach((file_data) => {
                if (file_data.search_string.includes(tag_id)) {
                    ...
                    const search_string = file_data.tags
                        .map((tag_id: string) =>
        ...
        deleteTagMetadata: (state, action) => {
            ...
            Object.entries(state.file_tags).forEach(([file_id, file_data]) => {
                ...
                if (file_data.search_string.includes(tag_id)) {
                    ...
                    file_data.tags = file_data.tags.filter(
                        (id: TagID) => id !== tag_id,
                    );
            ...
            console.log(
                "Deleted tag",
                tag_id,
                "from tag metadata and all files",
            );
        },
        ...
        addTagToFileID: (state, action) => {
            ...
            const search_string = state.file_tags[file_id].tags
                .map((tag_id: string) => generate_search_string(state, tag_id))
                .join(" ");
        ...
        removeTagFromFileID: (state, action) => {
            ...
            console.log(
                ...
                file_id,
                "tags",
                state.file_tags[file_id].tags,
            );
            state.file_tags[file_id].tags = state.file_tags[
                file_id
            ].tags.filter((id: TagID) => id !== tag_id);
...
const getModified = (state: RootState) => state.tags.modified;
const getTagMetadata = (state: RootState) => state.tags.tag_metadata;
const getTagFileID = (state: RootState) => state.tags.tag_file_id;
const getTagFileMetadata = (state: RootState) => state.tags.tag_file_metadata;
const getFileTags = (state: RootState) => state.tags.file_tags;
const getQueriedTags = (state: RootState) => state.tags.queried_tags;
const getModifyingTagData = (state: RootState) => state.tags.modifying_tag_data;

const getTagByID = (id: TagID) => (state: RootState) =>
    state.tags.tag_metadata[id];

const getFileTagsByID = (id: FileID) => (state: RootState) =>
    state.tags.file_tags[id];
