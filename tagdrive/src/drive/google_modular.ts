import {
    ...
    DriveListResponse,
    FileGenerateIdQuery,
    FileGenerateIdResponse,
    GoogleFile,
    FileCreateQuery,
    GoogleFileModifier,
    FileGetQuery as FileGetMetaDataQuery,
} from "./google_types";
...
class files_class {
    ...
    static async list(params: FileListQuery): Promise<FileListResponse> {
        ...
        const response = await fetch(
            files_class.#base_url + create_query_string(params),
            {
                method: "GET",
                headers,
            },
    ...
    static async get_metadata(
        file_id: string,
        params: FileGetMetaDataQuery,
    ): Promise<GoogleFile> {
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            files_class.#base_url + `/${file_id}` + create_query_string(params),
            {
                method: "GET",
                headers,
            },
    ...
    static async export(file_id: string, mimeType: string): Promise<string> {
        ...
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            files_class.#base_url +
                `/${file_id}/export` +
                create_query_string({ mimeType }),
            {
                method: "GET",
                headers,
            },
    ...
    static async generateIds(
        params: FileGenerateIdQuery,
    ): Promise<FileGenerateIdResponse> {
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        // Using this method https://stackoverflow.com/questions/68655145/create-folders-in-a-shared-drive-google-drive-api-with-javascript
    ...
    static async update(
        ...
        const blob_file = new Blob([file_content ? file_content : ""], {
            type: "text/plain",
        });
    ...
    static async create(
        ...
        form.append(
            "metadata",
            new Blob([JSON.stringify(file_metadata)], {
        ...
        // Using this method https://stackoverflow.com/questions/68655145/create-folders-in-a-shared-drive-google-drive-api-with-javascript
        const response = await fetch(
            files_class.#upload_url + create_query_string(params),
            {
                method: "POST",
                headers,
                body: form,
    ...
    static async create_no_content(
        file_metadata: GoogleFileModifier,
        params: FileCreateQuery,
    ): Promise<GoogleFile> {
...
class drives_class {
    ...
    static async list(params: DriveListQuery): Promise<DriveListResponse> {
        ...
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            drives_class.#url + create_query_string(params),
            {
                method: "GET",
                headers,
            },
...
class spreadsheets_values_class {
    ...
    static async get(
        ...
        range: string,
        body: SheetTypes.values.getRequestBody,
    ): Promise<SheetTypes.values.getResponseBody> {
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            spreadsheets_values_class.#base_url +
    ...
    static async batchGet(
        spreadsheetId: string,
        params: SheetTypes.values.batchGetQuery,
    ): Promise<SheetTypes.values.batchGetResponseBody> {
        const query =
            "&majorDimension=" +
            params.majorDimension +
            "&ranges=" +
            "[" +
            params.ranges.map((range) => `"${range}"`).join(",") +
            "]";
    ...
    static async append(
        ...
        range: string,
        body: SheetTypes.values.appendRequestBody,
    ) {
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
    ...
    static async batchUpdate(
        spreadsheet_id: string,
        body: SheetTypes.values.batchUpdateRequestBody,
    ): Promise<SheetTypes.values.batchUpdateResponseBody> {
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
...
class spreadsheets_class {
    ...
    static async get(
        spreadsheetId: string,
        params: SheetTypes.getSpreadsheetQuery,
    ): Promise<SheetTypes.Spreadsheet> {
        const query =
            "?includeGridData=" +
            params.includeGridData +
            params.ranges?.map((range) => "&ranges=" + range).join("");
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            spreadsheets_class.#base_url + `/${spreadsheetId}` + query,
            {
                method: "GET",
                headers,
            },
    ...
    static async batchUpdate(
        spreadsheetId: string,
        body: SheetTypes.batchUpdateRequestBody,
    ): Promise<SheetTypes.batchUpdateResponseBody> {
        const params = "?key=" + API_KEY;
        const headers = new Headers();
        headers.append("Authorization", `Bearer ${await getAuth()}`);
        headers.append("X-goog-api-key", API_KEY);
        const response = await fetch(
            spreadsheets_class.#base_url +
